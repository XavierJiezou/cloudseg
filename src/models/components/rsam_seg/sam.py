...
from src.models.components.rsam_seg.models import register
...
from src.models.components.rsam_seg.sammodel.transformer import TwoWayTransformer

logger = logging.getLogger(__name__)
from src.models.components.rsam_seg.iou_loss import IOU
...
class SAM(nn.Module):
    def __init__(
        ...
        self.image_embedding_size = inp_size // prompt_embed_dim
        self.no_mask_embed = nn.Embedding(1, prompt_embed_dim)

    def set_input(self, input: torch.Tensor, gt_mask: torch.Tensor):
        self.input = input
    ...
    def calculate_loss(self):
        ...
        # print(self.pred_mask.shape,self.gt_mask.shape,self.gt_mask.unsqueeze(1).shape)
        # print(self.pred_mask.dtype,self.gt_mask.unsqueeze(1).dtype)
        # print(self.pred_mask)
        self.loss_G = self.criterionBCE(self.pred_mask, self.gt_mask.unsqueeze(1).float())
        if self.loss_mode == "iou":
            self.loss_G += _iou_loss(self.pred_mask, self.gt_mask.unsqueeze(1).float())
